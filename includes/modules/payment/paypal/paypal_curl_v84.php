...
/**
 * PayPal NVP (v84.0) -- no longer compatible with: Payflow Pro (v4 HTTP API) implementation via cURL.
 *
 * lat9 - Vinos de Frutas Tropicales
 */
class paypal_curl_v84 extends base {
...
//                            CURLOPT_SSLVERSION => 3,  20141022-lat9-POODLE change, see http://www.zen-cart.com/showthread.php?214916-Important-announcement-about-POODLE-and-payment-security&p=1261900#post1261900
   ...
   */
  function paypal_curl_v84($params = array()) {
  ...
  function SetExpressCheckout($returnUrl, $cancelUrl, $options = array()) {
    ...
    $this->values = $values;
    $this->notify('NOTIFY_PAYPAL_SETEXPRESSCHECKOUT_BEFORE_REQUEST');
  ...
  function _request($values, $operation, $requestId = null) {
    ...
    // convert currency code to proper key name for nvp.  Variable name to set depends on the operation being performed.
    if ($this->_mode == 'nvp') {
  ...
  function _buildNameValueList($pairs) {
    ...
    foreach ($pairs as $name => $value) {
      ...
      if (($this->_mode == 'payflow') && (strpos($value, '&') !== false || strpos($value, '=') !== false)) {
        $name = str_replace('PAYMENTREQUEST_0_', '', $name);  // Remove NVP v63.0+ extras from name
