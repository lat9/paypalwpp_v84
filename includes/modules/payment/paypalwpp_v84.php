 ...
 */
require_once(DIR_FS_CATALOG . DIR_WS_MODULES . 'payment/paypal/paypal_curl_v84.php');
/**
 * the PayPal payment module with Express Checkout
 */
class paypalwpp_v84 extends base {
  /**
   * name of this module
   *
   * @var string
  ...
  function paypalwpp_v84() {
    ...
    global $order;
    $this->code = 'paypalwpp_v84';
    $this->codeTitle = MODULE_PAYMENT_PAYPALWPP_V84_TEXT_ADMIN_TITLE_EC;
    $this->codeVersion = '1.5.2';
    $this->enableDirectPayment = FALSE;
    $this->enabled = defined('MODULE_PAYMENT_PAYPALWPP_V84_STATUS') && MODULE_PAYMENT_PAYPALWPP_V84_STATUS == 'True';
    // Set the title & description text based on the mode we're in ... EC vs US/UK vs admin
    if (IS_ADMIN_FLAG === true) {
      $this->description = sprintf(MODULE_PAYMENT_PAYPALWPP_V84_TEXT_ADMIN_DESCRIPTION, ' (rev' . $this->codeVersion . ')');
      switch (MODULE_PAYMENT_PAYPALWPP_MODULE_MODE) {
        case ('PayPal'):
          $this->title = MODULE_PAYMENT_PAYPALWPP_V84_TEXT_ADMIN_TITLE_EC;
        break;
        case ('Payflow-UK'):
    ...
    $this->sort_order = MODULE_PAYMENT_PAYPALWPP_V84_SORT_ORDER;
    ...
    if (PROJECT_VERSION_MAJOR != '1' && (substr(PROJECT_VERSION_MINOR, 0, 3) != '5.1' || substr(PROJECT_VERSION_MINOR, 0,3) != '5.2') ) $this->enabled = false;
...
//-eof-v1.1.0a
      ...
      $this->notify('NOTIFY_PAYPALWPP_BEFORE_PROCESS_FINISHED', array ( 'response' => $response ));
  }
  /**
  ...
  function after_process() {
    ...
    $this->notify('NOTIFY_PAYPALWPP_AFTER_PROCESS_FINISHED');
  ...
  function admin_notification($zf_order_id) {
    if (!defined('MODULE_PAYMENT_PAYPALWPP_V84_STATUS')) return '';
    global $db;
    $output = '';

    require(DIR_FS_CATALOG . DIR_WS_MODULES . 'payment/paypal/paypalwpp_v84_admin_notification.php');
  ...
  function check() {
    ...
    if (!isset($this->_check)) {
      $check_query = $db->Execute("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_PAYMENT_PAYPALWPP_V84_STATUS'");
  ...
  function install() {
    ...
    if (defined('MODULE_PAYMENT_PAYPALWPP_V84_STATUS')) {
      $messageStack->add_session('Express Checkout (NVP 84.0) module already installed.', 'error');
      zen_redirect(zen_href_link(FILENAME_MODULES, 'set=payment&module=paypalwpp_v84', 'NONSSL'));
      return 'failed';
    }
    if (defined('MODULE_PAYMENT_PAYPALWPP_STATUS')) {
      $messageStack->add_session('<strong>Note:</strong> You already have the built-in Zen Cart Express Checkout module installed.  You must <b>remove</b> it if your want to install the Express Checkout (NVP 84.0) module.', 'error');
    ...
    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Enable this Payment Module', 'MODULE_PAYMENT_PAYPALWPP_V84_STATUS', 'True', 'Do you want to enable this payment module?', '6', '25', 'zen_cfg_select_option(array(\'True\', \'False\'), ', now())");
...
//    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Express Checkout: Require Confirmed Address', 'MODULE_PAYMENT_PAYPALWPP_CONFIRMED_ADDRESS', 'No', 'Do you want to require that your (not-logged-in) customers use a *confirmed* address when choosing their shipping address in PayPal?<br />(this is ignored for logged-in customers)', '6', '25',  'zen_cfg_select_option(array(\'Yes\', \'No\'), ', now())");
    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Express Checkout: Require Confirmed Address', 'MODULE_PAYMENT_PAYPALWPP_CONFIRMED_ADDRESS', 'No', 'Do you want to require that your customers use a <b>PayPal-confirmed</b> address when choosing their shipping address in PayPal? Choose one of:<ul><li><em>No</em> to never require a confirmed address</li><li><em>Yes</em> to require a confirmed address <b>only</b> if the customer is not logged into your store (i.e. they used the Express Checkout button)</li><li><em>Always</em> to <b>always</b> require a confirmed address (even if the customer is logged in).  If you are not collecting a full address during your checkout process, use of <em>Always</em> is suggested.</li></ul>', '6', '25',  'zen_cfg_select_option(array(\'Yes\', \'No\', \'Always\'), ', now())");
//-eof-pp84-a-lat9-20131229
    ...
    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Sort order of display.', 'MODULE_PAYMENT_PAYPALWPP_V84_SORT_ORDER', '0', 'Sort order of display. Lowest is displayed first.', '6', '25', now())");
...
//    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('PayPal Mode', 'MODULE_PAYMENT_PAYPALWPP_MODULE_MODE', 'PayPal', 'Which PayPal API system should be used for processing? <br /><u>Choices:</u><br /><font color=green>For choice #1, you need to supply <strong>API Settings</strong> above.</font><br /><strong>1. PayPal</strong> = Express Checkout with a regular PayPal account<br />or<br /><font color=green>for choices 2 &amp; 3 you need to supply <strong>PAYFLOW settings</strong>, below (and have a Payflow account)</font><br /><strong>2. Payflow-UK</strong> = Website Payments Pro UK Payflow Edition<br /><strong>3. Payflow-US</strong> = Payflow Pro Gateway only<!--<br /><strong>4. PayflowUS+EC</strong> = Payflow Pro with Express Checkout-->', '6', '25',  'zen_cfg_select_option(array(\'PayPal\', \'Payflow-UK\', \'Payflow-US\'), ', now())");
    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('PayPal Mode', 'MODULE_PAYMENT_PAYPALWPP_MODULE_MODE', 'PayPal', 'Which PayPal API system should be used for processing?', '6', '25',  'zen_cfg_select_option(array(\'PayPal\'), ', now())");
...
//-eof-v84c-lat9
  ...
  function paypal_init() {
    if (!defined('MODULE_PAYMENT_PAYPALWPP_V84_STATUS') || !defined('MODULE_PAYMENT_PAYPALWPP_SERVER')) {
      $doPayPal = new paypal_curl_v84(array('mode' => 'NOTCONFIGURED'));
      return $doPayPal;
    }
    ...
    if (substr(MODULE_PAYMENT_PAYPALWPP_MODULE_MODE,0,7) == 'Payflow') {
      $doPayPal = new paypal_curl_v84(array('mode' => 'payflow',
      ...
      $doPayPal = new paypal_curl_v84(array('mode' => 'nvp',
...
//        $subTotalTax += $optionsLI["L_PAYMENTREQUEST_0_TAXAMT$k"];
      ...
      $this->notify('NOTIFY_PAYPALWPP_GETLINEITEMDETAILS', array ( 'n' => 0, 'm' => $k ) );  // Made $optionsLI global for this notifier's use!
...
//      if (MODULE_PAYMENT_PAYPALWPP_CONFIRMED_ADDRESS != 'Yes' && ($address_arr = $this->getOverrideAddress()) !== false) {
      if (MODULE_PAYMENT_PAYPALWPP_CONFIRMED_ADDRESS == 'No' && ($address_arr = $this->getOverrideAddress()) !== false) {
//-eof-pp84c-lat9
        $address_error = false;
...
//-bof-wpp84c-lat9
      if (MODULE_PAYMENT_PAYPALWPP_CONFIRMED_ADDRESS == 'Always' || (MODULE_PAYMENT_PAYPALWPP_CONFIRMED_ADDRESS == 'Yes' && !(isset($_SESSION['customer_id']) && ((int)$_SESSION['customer_id']) > 0)) ) {  //-BUGFIX-20131213-lat9
//-eeof-wpp84c-lat9
...
///        $fromDb = str_replace("\n", '', $answers_arr->fields['entry_firstname'].$answers_arr->fields['entry_lastname']);
  ...
  function terminateEC($error_msg = '', $kill_sess_vars = false, $goto_page = '') {
    ...
    if (!empty($_SESSION['customer_first_name']) && !empty($_SESSION['customer_id'])) {
      if ($this->showPaymentPage === true || $goto_page == FILENAME_CHECKOUT_PAYMENT) {
        ...
        if ((!isset($_SESSION['shipping']) || $_SESSION['shipping'] == '') || $_SESSION['shipping']['cost'] < 0) {
          // debug
   ...
   * @return string
   *
   * NOTE: Was function zen_uncomment in paypal_curl.php for previous version
   */
